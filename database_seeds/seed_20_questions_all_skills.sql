-- ==============================================================================
-- TalentOrbit Production Database Migration: 20-Question Master Seed for All Skills
...
CREATE TABLE IF NOT EXISTS assessment_questions (
    ...
    tech_type VARCHAR(50) NOT NULL DEFAULT 'LANGUAGE',
...
-- ==============================================================================
-- 2. PYTHON — 20 Production Scenario Questions
-- ==============================================================================
...
('Python', NULL, 'LANGUAGE', 'Data Structures & Memory Mutability', 
 'What will be the output of executing the following Python code snippet?\n\na = [1, 2, [3, 4]]\nb = list(a)\nb[2].append(5)\nprint(len(a[2]))', 
 '2 (shallow copy isolates nested structures)', 
 '3 (shallow copy references the same inner mutable list)', 
 'Raises AttributeError', 
 'Raises TypeError on list() cast', 
 'B', 'list() creates a shallow copy. Nested mutable objects like [3, 4] are shared between both lists.'),

('Python', NULL, 'LANGUAGE', 'List & Dict Comprehensions', 
 'Which comprehension correctly generates a dictionary mapping even numbers from 0 to 10 to their squares?', 
 '{x: x**2 for x in range(11) if x % 2 == 0}', 
 '{x**2: x for x in range(10) if x % 2 == 0}', 
 'dict([x, x**2 for x in range(11) if x % 2 == 0])', 
 '{for x in range(11) if x % 2 == 0: x: x**2}', 
 'A', 'The syntax {k: v for item in iterable if condition} is valid dictionary comprehension.'),

('Python', NULL, 'LANGUAGE', 'Functions & Argument Unpacking', 
 'In Python, what is the key functional difference between *args and **kwargs in a function signature?', 
 '*args packs positional arguments into a tuple; **kwargs packs keyword arguments into a dictionary', 
 '*args packs keyword arguments into a list; **kwargs accepts default parameters', 
 '*args allows passing pointers; **kwargs passes by value', 
 'Both behave identically and can be used interchangeably', 
 'A', '*args gathers arbitrary positional arguments into a tuple, **kwargs gathers arbitrary keyword arguments into a dict.'),

('Python', NULL, 'LANGUAGE', 'OOP & Dunder Methods', 
 'In an object-oriented Python class, which dunder method should be implemented to return an unambiguous string representation primarily intended for developers and debugging?', 
 '__str__(self)', 
 '__repr__(self)', 
 '__debug__(self)', 
 '__format__(self)', 
 'B', '__repr__ is meant to be unambiguous and inspectable for developers, while __str__ is for end-user readability.'),

('Python', NULL, 'LANGUAGE', 'OOP & Method Decorators', 
 'What differentiates @classmethod from @staticmethod when applied to a method in a Python class?', 
 '@classmethod receives the class object (cls) as its first implicit parameter; @staticmethod receives no implicit first parameter', 
 '@classmethod cannot be called on class instances; @staticmethod can', 
 '@staticmethod can access private self variables; @classmethod cannot', 
 '@classmethod is executed during compile time; @staticmethod is executed during runtime', 
 'A', '@classmethod receives cls allowing alternative constructor patterns. @staticmethod is a plain function scoped in class namespace.'),

('Python', NULL, 'LANGUAGE', 'Generators & Memory Efficiency', 
 'When reading a multi-gigabyte log file line by line in Python, why is a generator function using yield preferred over returning a list of lines with readlines()?', 
 'yield produces values lazily one at a time, keeping memory footprint constant O(1)', 
 'yield automatically spawns a new OS process per line', 
 'yield compiles Python bytecode to C extensions', 
 'yield prevents file locks on the operating system', 
 'A', 'Generators evaluate items lazily on demand, keeping RAM consumption minimal regardless of file size.'),

('Python', NULL, 'LANGUAGE', 'Decorators & Closures', 
 'When creating a custom Python function decorator, why should you decorate the inner wrapper function with @functools.wraps(func)?', 
 'To preserve the original function name (__name__), docstring (__doc__), and signature metadata', 
 'To make the decorated function thread-safe', 
 'To automatically benchmark the execution time of the function', 
 'To bypass type hinting checks in Python', 
 'A', '@functools.wraps copies function attributes (name, docstring, annotations) to wrapper, preventing loss of introspection.'),

('Python', NULL, 'LANGUAGE', 'Exception Handling Architecture', 
 'In a Python try-except block, when is the code inside the else clause executed?', 
 'Only when an unhandled exception occurs in the try block', 
 'Only when the try block completes successfully without any exception being raised', 
 'Always, right before the finally clause', 
 'Only if a specified fallback exception is caught', 
 'B', 'The else block in try-except executes exclusively when the try block finishes without raising any exceptions.'),

('Python', NULL, 'LANGUAGE', 'Context Managers & Resource Cleanup', 
 'Which two dunder methods must a Python class implement to support the with statement as a context manager?', 
 '__open__ and __close__', 
 '__enter__ and __exit__', 
 '__start__ and __stop__', 
 '__init__ and __del__', 
 'B', 'A context manager must implement __enter__(self) and __exit__(self, exc_type, exc_val, exc_tb).'),

('Python', NULL, 'LANGUAGE', 'Concurrency & the GIL', 
 'In standard CPython, what is the primary consequence of the Global Interpreter Lock (GIL) when running CPU-bound multithreaded tasks?', 
 'CPython threads run on separate CPU cores concurrently without restrictions', 
 'Only one native thread can execute Python bytecode at a time, limiting CPU-bound parallelism', 
 'The GIL prevents threads from making I/O network calls', 
 'The GIL automatically converts multithreaded code to asynchronous coroutines', 
 'B', 'CPython uses the GIL for memory safety. For CPU-bound parallel workloads, multiprocessing is required.'),

('Python', NULL, 'LANGUAGE', 'Asynchronous Programming (asyncio)', 
 'What keyword must be used before an asynchronous function invocation to pause execution of the current coroutine until the result is ready?', 
 'yield from', 
 'await', 
 'defer', 
 'async', 
 'B', 'The await keyword pauses coroutine execution until the awaited Awaitable yields a result.'),

('Python', NULL, 'LANGUAGE', 'Type Hinting & Static Analysis', 
 'In modern Python (3.10+), which syntax correctly expresses a type hint for a function argument that can either be an integer or None?', 
 'val: int | None', 
 'val: Either[int, None]', 
 'val: int or None', 
 'val: int || None', 
 'A', 'Python 3.10 introduced the union operator | for type hints, allowing int | None as an alternative to Optional[int].'),

('Python', NULL, 'LANGUAGE', 'Collections & Performance', 
 'In the Python standard library collections module, what data structure provides O(1) time complexity for appending and popping elements from both ends?', 
 'collections.deque', 
 'collections.defaultdict', 
 'collections.OrderedDict', 
 'collections.Counter', 
 'A', 'collections.deque guarantees O(1) appends and pops from both ends, unlike list which has O(n) left pops.'),

('Python', NULL, 'LANGUAGE', 'Memory Optimization (__slots__)', 
 'What is the primary benefit of defining __slots__ = ("id", "name") inside a Python class that will be instantiated millions of times?', 
 'Prevents creation of a dynamic __dict__ per instance, drastically reducing memory consumption', 
 'Encrypts the class attributes in memory', 
 'Enforces compile-time type validation for the attributes', 
 'Enables automatic database persistence of the instances', 
 'A', 'By default, Python objects allocate a dynamic __dict__. Defining __slots__ allocates a fixed array, saving significant RAM.'),

('Python', NULL, 'LANGUAGE', 'String Manipulation & Formatting', 
 'Which Python f-string formatting specifier rounds the floating point variable val = 3.1415926 to two decimal places?', 
 'f"{val:.2f}"', 
 'f"{val:round(2)}"', 
 'f"{val%2f}"', 
 'f"{round(val, 2f)}"', 
 'A', 'f"{val:.2f}" formats floating point numbers with 2 digits of precision after the decimal.'),

('Python', NULL, 'LANGUAGE', 'Structural Pattern Matching', 
 'In Python 3.10+, which statement introduced structural pattern matching with case de-structuring?', 
 'switch / case', 
 'match / case', 
 'select / case', 
 'when / then', 
 'B', 'Python 3.10 introduced match / case syntax (PEP 634) for structural pattern matching.'),

('Python', NULL, 'LANGUAGE', 'Testing & Mocking with pytest', 
 'In pytest, what decorator is used to define reusable setup/teardown dependencies injected into test functions?', 
 '@pytest.setup', 
 '@pytest.fixture', 
 '@pytest.inject', 
 '@pytest.dependency', 
 'B', '@pytest.fixture defines a fixture function that supplies reusable instances or state into test parameters.'),

('Python', NULL, 'LANGUAGE', 'Working with JSON & Serialization', 
 'In the standard json module, which function converts a valid JSON-formatted string into a corresponding Python dictionary or list?', 
 'json.dumps()', 
 'json.loads()', 
 'json.parse()', 
 'json.decode()', 
 'B', 'json.loads() (load string) deserializes JSON strings into native Python objects.'),

('Python', NULL, 'LANGUAGE', 'Packaging & Environment Isolation', 
 'What file in a Python project specifies exact pinned package versions to guarantee reproducible builds across environments with pip install -r?', 
 'requirements.txt', 
 'setup.cfg', 
 'Pipfile.lock', 
 'pyproject.toml', 
 'A', 'requirements.txt specifies pinned package versions consumed by pip install -r requirements.txt.'),

('Python', NULL, 'LANGUAGE', 'Security & Database Best Practices', 
 'When querying a relational database using Python DB-API (such as sqlite3 or psycopg2), what is the industry-standard way to prevent SQL Injection attacks?', 
 'Using formatted f-strings: cursor.execute(f"SELECT * FROM users WHERE id = {uid}")', 
 'Using parameterized placeholders: cursor.execute("SELECT * FROM users WHERE id = %s", (uid,))', 
 'Using string concatenation with .replace()', 
 'Using eval() before executing the raw SQL string', 
 'B', 'Parameterized queries separate SQL command logic from input data, preventing injection attacks.');
